...
def _parse_args():
    ...
    args = config_parser.parse_args(remaining)
...
def create_datasets_and_loaders(
    ...
    def __init__(self,annotations):
        self.annotations=self.load_data(annotations)
    ...
    def __len__(self)->int:
        """To find the length of a data"""
        length=len(self.annotations["samples"])
        return length
    def convert_bbox(self,loc,image)->list:
        """convert udt bbox to pascalvoc bbox"""
        xmin=(loc["centerX"]-loc['width']*0.5)*image.shape[1]
        ymin=(loc['centerY']-loc['height']*0.5)*image.shape[0]
        xmax=loc["width"]*image.shape[1]
        ymax=loc["height"]*image.shape[0]
        pascal_bbox=[xmin,ymin,xmax,ymax]
        return pascal_bbox
    def __getitem__(self,idx)->list:
        """To return images and respective labels"""
        path=self.annotations["samples"][idx]["imageUrl"]
        #For Macos I have replaced the url format
        path=path.replace('file://','')
        path=path.replace(os.sep,'//')
        image=read_image(path)
        c,new_h,new_w=image.shape
        annotate=self.annotations["samples"][idx]["annotation"]
        bboxes=[]
        class_labels=[]
        image_id=[]
        
        for numb in range(len(annotate)):
            loc=self.annotations["samples"][idx]["annotation"][numb]
            le = LabelEncoder()
            label=self.convert_bbox(loc,image)
            bboxes.append(label)
            class_label=[loc['classification']]
            class_label = le.fit_transform(class_label)
        ...
        target = {
            ...
            "labels": torch.as_tensor([class_labels]),
    ...
    your_dataset = create_datasets_and_loaders(annotations)
    loader_train=DataLoader(dataset=your_dataset,batch_size=2,shuffle=True,collate_fn=lambda x:x)
    loader_train=iter(loader_train)
    data=loader_train.next()
    images,labels=data
    print(images,labels)
    print(your_dataset.convert_bbox(1))
    return loader_train
